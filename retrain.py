...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
    ...
    with tf.name_scope(layer_name):
        ...
        with tf.name_scope("layer_1"):
            W1 = tf.Variable(tf.truncated_normal([bottleneck_tensor_size, 500], stddev=0.1), name='W1')
            variable_summaries(W1)
            b1 = tf.Variable(tf.zeros([500]) + 0.1, name='b1')
            variable_summaries(b1)
            z1 = tf.matmul(bottleneck_input, W1, name='z1') + b1
            a1 = tf.nn.relu(z1, name='a1')
            a1_drop = tf.nn.dropout(a1, keep_prob=keep_prob, name='a1_drop')
        with tf.name_scope("layer_2"):
            W2 = tf.Variable(tf.truncated_normal([500, 300], stddev=0.1), name='W2')
            variable_summaries(W2)
            b2 = tf.Variable(tf.zeros([300]) + 0.1, name='b2')
            variable_summaries(b2)
            z2 = tf.matmul(a1_drop, W2, name='z2') + b2
            a2 = tf.nn.relu(z2, name='a2')
            a2_drop = tf.nn.dropout(a2, keep_prob=keep_prob, name='a2_drop')
        with tf.name_scope("layer_3"):
            W3 = tf.Variable(tf.truncated_normal([300, 250], stddev=0.1), name='W3')
            variable_summaries(W3)
            b3 = tf.Variable(tf.zeros([250]) + 0.1, name='b3')
            variable_summaries(b3)
            z3 = tf.matmul(a2_drop, W3, name='z3') + b3
            a3 = tf.nn.relu(z3, name='a3')
            a3_drop = tf.nn.dropout(a3, keep_prob=keep_prob, name='a3_drop')
        with tf.name_scope("layer_4"):
            W4 = tf.Variable(tf.truncated_normal([250, class_count], stddev=0.1), name='W4')
            variable_summaries(W4)
            b4 = tf.Variable(tf.zeros([class_count]) + 0.1, name='b4')
            variable_summaries(b4)
            z4 = tf.matmul(a3_drop, W4, name='z4') + b4
            # a4 = tf.nn.softmax(z4, name='a4')
            tf.summary.histogram('pre_activations', z4)
    ...
    final_tensor = tf.nn.softmax(z4, name=final_tensor_name)
    # final_tensor = tf.nn.dropout(a4, keep_prob=keep_prob, name=final_tensor_name)

    tf.summary.histogram('activations', final_tensor)

    with tf.name_scope('cross_entropy'):
        cross_entropy_mean = tf.losses.sparse_softmax_cross_entropy(
            labels=ground_truth_input, logits=z4)

        # y = tf.one_hot(ground_truth_input, depth=class_count)
        # cross_entropy_mean = tf.reduce_mean(
        #     tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=final_tensor))
...
def create_model_info(architecture):
    ...
    if architecture == 'inception_v3':
        ...
        # model_file_name = 'baggage_graph.pb'
        input_mean = 128
        input_std = 128
        ...
        for i in range(FLAGS.how_many_training_steps):
            ...
            train_summary, _ = sess.run(
                ...
                feed_dict={bottleneck_input: train_bottlenecks,
                           ground_truth_input: train_ground_truth, keep_prob: 0.7})
...
if __name__ == '__main__':
    image_dir = 'D:/retrain/retrain_data_color/backpack/train_data'
    model_dir = 'D:/MyCode/Python/tensorflow_L/inception_model/'
    bottleneck_dir = 'D:/retrain/retrain_data_color/backpack/bottleneck'
    summaries_dir = 'D:/retrain/retrain_data_color/backpack/retrain_logs/mine'
    output_graph = 'D:/retrain/retrain_data_color/backpack/backpack_color_mine.pb'
    output_labels = 'D:/retrain/retrain_data_color/backpack/backpack_color_labels_mine.txt'
